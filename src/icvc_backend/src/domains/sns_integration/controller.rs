use crate::{
    utils::authenticator::check_is_owner, APIError, ListProposalsResponse, ProposalData,
    ProposalPayload,
};
...
#[ic_cdk::update]
fn validate_project_vote_proposal(proposal_payload: ProposalPayload) -> Result<String, String> {
    let caller_id = ic_cdk::caller();

    match check_is_owner(caller_id) {
...
#[ic_cdk::update]
async fn execute_project_vote_proposal(proposal_payload: ProposalPayload) {
    let caller_id = ic_cdk::caller();
